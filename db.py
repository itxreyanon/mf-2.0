...
# --- Cache Setup ---
# This creates a shared cache for user-specific data.
# maxsize=1024: Stores data for up to 1024 different users.
# ttl=15: Data for each user expires after 15 seconds, forcing a refresh from the database.
user_data_cache = TTLCache(maxsize=1024, ttl=15)

# --- Async Cache Decorator ---
# A simple wrapper to make the synchronous cachetools library work with async functions.
def async_cached(cache):
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            # Create a unique key for the function call and its arguments
            key = (func.__name__,) + args + tuple(sorted(kwargs.items()))
            try:
                # If the result is in the cache, return it instantly
                return cache[key]
            except KeyError:
                # If not, run the function, store the result in the cache, and then return it
...
# Any function that MODIFIES data should clear the cache to ensure outdated information isn't served.
...
async def delete_token(telegram_user_id, token):
    ...
    # Re-fetch from DB, not cache, to check if the deleted token was current
    ...
    if settings and settings.get("current_token") == token:
        await set_current_account(telegram_user_id, None) # This will clear cache again
...
async def toggle_token_status(telegram_user_id, token):
    ...
    await _ensure_user_collection_exists(telegram_user_id)
    user_db = _get_user_collection(telegram_user_id)
    # Must fetch the current state directly to toggle it
...
# These functions will hit the database only if the data is not in the cache or has expired.
...
async def connect_to_collection(collection_name, target_user_id):
    user_data_cache.clear() # This is a major write operation

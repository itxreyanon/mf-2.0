        ...
        if data == "signup_settings":
            ...
            if not all(k in config for k in ['email', 'password', 'gender', 'birth_year', 'nationality']):
                try:
                    ...
                    logger.warning(f"TelegramBadRequest in signup_go: {e}")
                ...
                pending_emails = [acc['email'] for acc in state.get('pending_accounts', [])]
                available_email_count = await count_available_emails(config.get('email', ''), pending_emails, max_check=50)
                try:
                    await callback.message.edit_text(
                        f"<b>Account Creation</b>\n\n"
                        f"Enter the number of accounts to create (1-10).\n"
                        f"Available email variations for <code>{config.get('email', 'Not set')}</code>: {available_email_count}",
...
async def signup_message_handler(message: Message) -> bool:
    ...
    if stage == "change_email":
        ...
        try:
            ...
            available_email_count = await count_available_emails(config.get('email', ''), pending_emails, max_check=50)
            await message.answer(
                f"<b>Invalid number (1-10). Please try again:</b>\n\n"
                f"Available email variations for <code>{config.get('email', 'Not set')}</code>: {available_email_count}",
                reply_markup=BACK_TO_SIGNUP,
                parse_mode="HTML"
            )
